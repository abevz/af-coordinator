...
- Concurrent instances of the same tool MUST use distinct names: identical holders make the audit trail ambiguous and the one-claim rule unverifiable. Easiest: derive the suffix from the session PID once, in the launcher — `export AF_COORDINATOR_ACTOR="antigravity-$$"` (never per-command: that fragments the trail; and PIDs recycle across reboots, so read old events with their timestamps).
